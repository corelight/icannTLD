...
export {
	...
	option test_query: string = "1.splunk.lagermann.net";
}
...
## begin added for testing only (remove for production)
	...
	if ( end_of_data_count == 4 ) {
		...
		print fmt("Time: %.6f", end_time - start_time) +" " +c$dns$query, c$dns$icann_host_subdomain, c$dns$icann_domain, c$dns$icann_tld, c$dns$is_trusted_domain;
		terminate();
		exit(0);
	}
## end added for testing only (remove for production)
